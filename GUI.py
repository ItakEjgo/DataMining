...
def chose_directory(title):
    root = Tk()
    root.filename = filedialog.askdirectory(initialdir="/", title=title)
    ret = root.filename
    root.destroy()
    return ret + '/'
...
if __name__ == '__main__':
    ...
    for algo, num in options:
        Radiobutton(window, text=algo, variable=selected, value=num, font=('Calibri', 15)).pack(anchor=W)
        i += 1

    txt = scrolledtext.ScrolledText(window, width=600, height=60, font=('Calibri', 15))
    ...
    def clicked():
        ...
        if chosen == 4:
            txt.delete(1.0, END)
            path1 = chose_file('Choose the first stocks')
            path2 = chose_file('Choose the second stocks')
